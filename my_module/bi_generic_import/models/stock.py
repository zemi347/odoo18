                ...
                if i != 0:
                    ...
                    if bool(val):
                        ...
                        if self.lot_option == True:
                            ...
                            search_line = self.env['stock.quant'].search(
                                [('product_id', '=', val['product']), ('location_id', '=', stock_location_id.id)])
                            _logger.info("Hammad debug | stock.quant " + str(search_line))
                            
                            if search_line:
                                for stock_line_id in search_line:
                                    stock_line_id.write({'inventory_quantity': val['quantity']})
                                    _logger.info("Hammad debug | stock.quant 2 " + str(search_line))
                                    stock_line_id.action_apply_inventory()

                            else:
                                _logger.info("Hammad debug | stock.quant 3 " + str(search_line))
                                if self.location_id_option == True:
                                    _logger.info("Hammad debug | stock.quant 4 " + str(search_line))
                                    stock_line_id = inventory_obj.create(
                                        {'product_id': val['product'], 'location_id': stock_location_id.id,
                                         'product_uom_id': product_uom_id.id, 'inventory_quantity': val['quantity'],
                                         'is_import': True})
                                else:
                                    _logger.info("Hammad debug | stock.quant 5 " + str(search_line))
                                    stock_line_id = inventory_obj.create(
                                        {'product_id': val['product'],
                                         'product_uom_id': product_uom_id.id, 'inventory_quantity': val['quantity'],
                                         'is_import': True})

                                _logger.info("Hammad debug | stock.quant 6 " + str(search_line))
                                stock_line_id.action_apply_inventory()
